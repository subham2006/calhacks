...
function SvgExport({
  ...
  const [defsById, setDefsById] = (0, import_react.useState)({});
  const addExportDef = (0, import_useEvent.useEvent)((def) => {
    if ((0, import_utils.hasOwnProperty)(defsById, def.key)) return;
    waitUntil(
      (async () => {
        const element = await def.getElement();
        if (!element) return;
        (0, import_react_dom.flushSync)(() => setDefsById((defsById2) => ({ ...defsById2, [def.key]: element })));
      })()
    );
  });
  ...
  const [shapeElements, setShapeElements] = (0, import_react.useState)(null);
      ...
      const unorderedShapeElementPromises = renderingShapes.map(
        async ({ id, opacity, index, backgroundIndex }) => {
          ...
          if (util.toSvg || util.toBackgroundSvg) {
            ...
            const shapeMaskId = `mask_${(0, import_useSafeId.sanitizeId)(shape.id)}`;
            if (shapeMask) {
              shapeDefs[shapeMaskId] = /* @__PURE__ */ (0, import_jsx_runtime.jsx)("clipPath", { id: shapeMaskId, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: `M${shapeMask.map(({ x, y }) => `${x},${y}`).join("L")}Z` }) });
            }
            if (toSvgResult) {
              elements.push({
                zIndex: index,
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_SvgExportContext.SvgExportContextProvider, { editor, context: exportContext, children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
    "svg",
    {
      ...
      children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("defs", { children: Object.entries(defsById).map(([key, def]) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_react.Fragment, { children: def }, key)) }),
        shapeElements
      ]
    }
  ) });
